...
void Tga::PutToImage(NtImage* image)
{
	int imgH = image->GetHeight();
	int imgW = image->GetWidth();
	for (int y = 0; y < imgH; y++)
	{
		float ty = float(y) / imgH;
		int tgaY = ty*height;
		for (int x = 0; x < imgW;x++) {
			float tx = float(x) / imgW;
			int tgaX = tx * width;
			image->SetPixel(x, y, GetPixel(tx, ty));
...
void TGAImage::PutToImage(NtImage* image)
{
	int imgW = image->GetWidth();
	int imgH = image->GetHeight();
	scale(imgW, imgH);
	for (int i = 0; i < imgH; i++)
	{
		...
		{
			image->SetPixel(j,i,this->get(j, i).bgra32());
		}
	}
}
*/